...
async function awardXPForMatch(matchId) {
  ...
  console.log(`Both Confirmed: Home=${match.homeCaptainConfirmed}, Away=${match.awayCaptainConfirmed}`);
...
// Award XP for the latest match that has both captains confirmed but XP not awarded
async function main() {
  // Find matches where both captains confirmed but stats have xp_awarded = 0
  const [matchesNeedingXP] = await sequelize.query(`
    SELECT DISTINCT m.id, m.status, m."homeCaptainConfirmed", m."awayCaptainConfirmed"
    FROM "Matches" m
  ...
  console.log(`Found ${matchesNeedingXP.length} matches needing XP award\n`);
  
  for (const match of matchesNeedingXP) {
    await awardXPForMatch(match.id);
    console.log('\n---\n');
  }
